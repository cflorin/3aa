...
- `ClassificationResult` shape: `{ suggested_code: string | null, bucket: 1|2|3|4|5|6|7|8 | null, eq_grade: 'A'|'B'|'C' | null, bs_grade: 'A'|'B'|'C' | null, confidence_level: 'high'|'medium'|'low', reason_codes: string[], scores: { bucket: Record<1–8, number>, eq: Record<'A'|'B'|'C', number>, bs: Record<'A'|'B'|'C', number> }, missing_field_count: number }`
...
- [ ] `src/domain/classification/confidence-thresholds.ts` constants implemented (per ADR-014)
