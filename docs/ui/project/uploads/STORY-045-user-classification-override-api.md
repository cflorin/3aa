...
- `DELETE /api/classification-override/[ticker]` (authenticated):
  ...
  - 204 on success; 200 with active code falling back to system suggestion
...
- `DELETE`: 204 no body (or 200 with reverted classification)
...
- [ ] Unknown ticker → 404
- [ ] `DELETE /api/classification-override/[ticker]` removes row; `active_code` falls back to `system_suggested_code`
