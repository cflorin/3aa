...
/** Runs syncForwardEstimates with a given epsTtm in the DB row and nonGaapEpsMostRecentFy from estimates */
async function runSync(epsTtm: number | null, nonGaapEps: number | null) {
...
describe('EPIC-003/STORY-031/TASK-031-006: gaapAdjustmentFactor computation', () => {
  ...
  it('basic ratio: epsTtm=4.0, nonGaapEps=5.0 → factor=0.8', async () => {
    const factor = await runSync(4.0, 5.0);
    expect(Number(factor)).toBeCloseTo(0.8, 5);
  });

  it('typical AAPL: epsTtm=6.13, nonGaapEps=7.20 → factor≈0.8514', async () => {
    const factor = await runSync(6.13, 7.20);
    expect(Number(factor)).toBeCloseTo(6.13 / 7.20, 4);
  });

  it('clamps to lower bound: epsTtm=0.02, nonGaapEps=0.5 → raw=0.04 → clamped to 0.10', async () => {
    const factor = await runSync(0.02, 0.5);
    expect(Number(factor)).toBeCloseTo(0.10, 5);
  });

  it('clamps to upper bound: epsTtm=3.0, nonGaapEps=1.0 → raw=3.0 → clamped to 2.00', async () => {
  ...
  it('returns null when epsTtm is null (no GAAP EPS in DB)', async () => {
