...
- **Cross-story E2E workflows:**
  ...
  3. **Filter + sort workflow:** Apply text search → stocks narrow → apply sector filter → stocks narrow further → sort by Market Cap descending → verify top stock has highest market cap → clear filters → all stocks return
  ...
  7. **Stock detail page:** Click MSFT row in universe screen → navigates to `/stocks/MSFT` → Classification tab renders with score bars, confidence steps, tie-break analysis, E1–E6 star ratings → switch to Fundamentals tab → metrics shown with color coding → switch to Valuation tab → placeholder text shown → switch to History tab → classification history timeline or empty state shown → "← Universe" button returns to universe screen
...
## Preconditions
- All STORY-041–051 and STORY-053 implemented and unit/integration tests passing
...
## Acceptance Criteria
- [ ] All 7 cross-story E2E workflows implemented and passing
