  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // Stage 6: flags
        currentStage = 'flags';
        emit({ stage: 'flags', label: 'Computing classification flags…', step: 6, total: TOTAL_STAGES });
        await syncDeterministicClassificationFlags({ tickerFilter: ticker });

        // Stage 7: enrichment (LLM — longest stage)
        currentStage = 'enrichment';
        emit({ stage: 'enrichment', label: 'Running LLM enrichment…', step: 7, total: TOTAL_STAGES });
        await syncClassificationEnrichment(llm, { mode: 'full', tickerFilter: ticker });

        // Stage 8: classification
        currentStage = 'classification';
        emit({ stage: 'classification', label: 'Classifying…', step: 8, total: TOTAL_STAGES });
